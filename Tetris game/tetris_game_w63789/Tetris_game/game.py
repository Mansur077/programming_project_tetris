...
def combine_matrices(mat1, mat2, mat2_off):
...
def display_menu(screen, menu, x_pos = 100, y_pos = 100, font = None,
            size = 70, distance = 1.4, fgcolor = (255,255,255),
            cursorcolor = (255,0,0), exitAllowed = True):

	pygame.font.init()
	if font == None:
		myfont = pygame.font.Font('tfont.ttf', size)
	else:
		myfont = pygame.font.SysFont('tfont.ttf', size)
	cursorpos = 0
...
class TetrisApp(object):
    ...
    def drop(self, manual):
        if not self.gameover and not self.paused:
            ...
            if is_collision(self.board, self.stone,(self.stone_x, self.stone_y)):
                self.board = combine_matrices(self.board, self.stone, (self.stone_x, self.stone_y))
                self.new_stone()
                cleared_rows = 0
                while True:
    ...
    def shadow(self, manual):
        if not self.gameover and not self.paused:
            ...
            if is_collision(self.board, self.stone,(self.stone_x, self.stone_y_shadow)):
    ...
    def rotate_stone(self):
        if not self.gameover and not self.paused:
            new_stone = rotate_shape_clockwise(self.stone)
            if not is_collision(self.board,
                                   new_stone,
    ...
    def run(self):
        ...
        while 1:
            ...
            if self.gameover:
                self.center_message("""Game Over!\nYour score: %d # Press ENTER to continue""" % self.score)
            else:
                if self.paused:
                    self.center_message("Paused")
                else:
...
class Menu(object):
    def __init__(self):
        ...
        self.default_font =  pygame.font.Font('tfont.ttf', 16)
    ...
    def main(self):
        ...
        choose = display_menu(self.screen, [
                            'Start Game',
                            str(screen_state),
